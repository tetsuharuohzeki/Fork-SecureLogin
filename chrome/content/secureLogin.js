...
var secureLogin = {
	...
	setExceptions: function (aExceptions) {
		...
		this.secureLoginPrefs.setComplexValue(
			...
			this.getUnicodeString(aExceptions.join(' '))
		);
	},
	...
	applyShortcut: function (aEvent, aId) {
		...
		if (aEvent.view.document && aEvent.view.document.getElementById(aId)) {
			aEvent.view.document.getElementById(aId).value = this.getFormattedShortcut(shortcut);
		}
	},
	...
	selectAudioFile: function (aDoc, aPrefName) {
		...
		try {
			...
			if (rv == Components.interfaces.nsIFilePicker.returnOK) {
				...
				this.secureLoginPrefs.setComplexValue(aPrefName, Components.interfaces.nsILocalFile, file);
